...
class MultiMagnification_Nonrigid_Network(nn.Module):
    def __init__(self, device):
        super(MultiMagnification_Nonrigid_Network, self).__init__()
        self.device = device
    ...
    def forward(self, source, target):
        # source: (n_samples, 3, 256, 256)
        # target: (n_samples, 3, 256, 256)
...
def load_network(device, path=None):
    model = MultiMagnification_Nonrigid_Network(device=device)

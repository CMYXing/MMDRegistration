...
from network import mm_local_affine_network as mmla
...
def training():
    """
    Train the multi-magnification local affine registration network
    """
    ...
    parser.add_argument('--stride', dest="stride", type=int, default=128, help="Stride of extraction of patch sets")
    parser.add_argument('--patch_size', dest="patch_size", type=int, default=256, help="Input patch size of network")
    ...
    parser.add_argument('--inner_iterations', dest="inner_iterations", type=int, default=1, help="Num of iterations per registration")
    parser.add_argument('--initial_model', dest="initial_model", type=str, default=None, help="The pre-trained model name")

    # data setting
    parser.add_argument('--train_dir', dest="train_dir", type=str, default=paths.la_train_folder, help="Directory for the training data")
    parser.add_argument('--val_dir', dest="val_dir", type=str, default=paths.la_val_folder, help="Directory for the validation data")
    ...
    parser.add_argument('--min_translation', dest="min_translation", type=float, default=-0.001, help="The minimum allowed translation distance")
    parser.add_argument('--max_translation', dest="max_translation", type=float, default=0.001, help="The maximum allowed translation distance")
    parser.add_argument('--min_rotation', dest="min_rotation", type=float, default=-1, help="The minimum allowed rotation angle")
    parser.add_argument('--max_rotation', dest="max_rotation", type=float, default=1, help="The maximum allowed rotation angle")
    ...
    # save path setting
    parser.add_argument('--model_name', dest="model_name", type=str, default="local_affine_model", help="Model name")
    parser.add_argument('--model_dir', dest="model_dir", type=str, default=paths.la_model_folder, help="Directory for saving the models")
    ...
    initial_model = args.initial_model  # initial model path
    if initial_model is not None:
        initial_model = os.path.join(args.model_dir, initial_model)

    train_folder = args.train_dir  # training data path
    val_folder = args.val_dir  # validation data path

    # saving files path
    model_name = args.model_name
    model_save_folder = os.path.join(args.model_dir, curr_time)
    logging_folder = os.path.join(args.log_dir, "local_affine_network")
    ...
    ### Training setting
    model = mmla.load_network(device, path=initial_model)
            ...
            for i in range(len(sources)):
                ...
                for inner_iter in range(inner_iterations):
                    ...
                    for j in range(iters):
                        with torch.set_grad_enabled(False):
                            ...
                            if 0 in torch.std(mm_sps[:, 0, :, :], dim=(1, 2)) or 0 in torch.std(mm_tps[:, 0, :, :], dim=(1, 2)):
                                all_displacement_fields = torch.cat(
                                    (all_displacement_fields, torch.zeros(
                                        (mm_sps.size(0), 2, int(mm_sps.size(2) / cropping_rate), int(mm_sps.size(3) / cropping_rate))).to(device)))
                                continue
                        ...
                        with torch.set_grad_enabled(True):
                            transforms = model(mm_sps, mm_tps)  # (num_of_patches, 2, 3)
                            displacement_fields = utils.transform_to_displacement_fields(mm_sps, transforms, device=device)
                            ...
                            if stride != patch_size:
                                cropped_displacement_fields = utils.center_crop_tensor(
                                    displacement_fields.clone(), cropping_rate=cropping_rate, device=device)
                            ...
                            loss = cost_func(mm_tsps, mm_tps, device, **cost_func_params)
                    ...
                    with torch.set_grad_enabled(False):
                        ...
                        all_displacement_fields = utils.restore_displacement_field(
                            all_displacement_fields, (target.size(0), target.size(1)), padding_tuple, stride, device=device)
                        ...
                        inner_displacement_field = utils.compose_displacement_fields(
                            inner_displacement_field, smoothed_displacement_field, device=device)
...
def local_affine_registration(source, target, model, inner_iterations, device=torch.device("cpu")):

    print("\n[INFO] Local affine registration")

    stride = Parameter.get_value("la_stride")
    patch_size = Parameter.get_value("la_patch_size")
    magnifications_num = Parameter.get_value("la_magnifications_num")
    num_of_patches = 4
    ...
    for inner_iter in range(inner_iterations):
        ...
        for j in range(iters):
            with torch.set_grad_enabled(False):
                ...
                transforms = model(mm_sps, mm_tps)
                displacement_fields = utils.transform_to_displacement_fields(mm_sps, transforms, device=device)

                if stride != patch_size:
                    cropped_displacement_fields = utils.center_crop_tensor(
                        displacement_fields.clone(), cropping_rate=int(patch_size / stride), device=device)
                    all_displacement_fields = torch.cat((all_displacement_fields, cropped_displacement_fields))
                else:
                    all_displacement_fields = torch.cat((all_displacement_fields, displacement_fields))
